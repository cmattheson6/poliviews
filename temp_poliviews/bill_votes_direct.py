...
### DELETE THIS
# The following is mock data required to build and execute a PubSubMessage for testing the pipeline.
# It will run the mock message through PubSub and back into this Dataflow Pipeline.

# Necessary to convert the message string to unicode
x_data = u'This is a representative in the House.'
x_data = x_data.encode('utf-8')

# The PubSubMessage accepts a dictionary for its attributes, but only as strings.
# It will be converted in the next step.
x_attributes = {
     'bill_id': 'S3600',
     'amdt_id': None,
     'first_name': 'Joe',
...
# Converting here.
x_attributes = {k:str(v) for (k,v) in x_attributes.items()}
x_attributes_w_error = {k:str(v) for (k,v) in x_attributes_w_error.items()}

# This is a mock nickname table to test the nickname mapping DoFn.
n_tbl_ex = [{'nickname':'Joe', 'full_name': 'Joseph'}, {'nickname':'Carol', 'full_name':'Caroline'}]

logging.info('Created test datasets.')
...
# Publish the test PubSubMessage
topic_path = publisher.topic_path(project_id, topic_name)
future = publisher.publish(topic_path, data=x_data,
                           bill_id = x_attributes['bill_id'],
                           amdt_id = x_attributes['amdt_id'],
                           first_name = x_attributes['first_name'],
                           last_name = x_attributes['last_name'],
                           state = x_attributes['state'],
                           party = x_attributes['party'],
                           vote_cast = x_attributes['vote_cast'],
                           vote_date = x_attributes['vote_date'],
                           chamber = x_attributes['chamber'])
future = publisher.publish(topic_path, data=x_data,
                           bill_id = x_attributes_w_error['bill_id'],
                           amdt_id = x_attributes_w_error['amdt_id'],
                           first_name = x_attributes_w_error['first_name'],
                           last_name = x_attributes_w_error['last_name'],
                           state = x_attributes_w_error['state'],
                           party = x_attributes_w_error['party'],
                           vote_cast = x_attributes_w_error['vote_cast'],
                           vote_date = x_attributes_w_error['vote_date'],
                           chamber = x_attributes_w_error['chamber'])

# Return the successful PubSubMessage ID in order to confirm publishing
